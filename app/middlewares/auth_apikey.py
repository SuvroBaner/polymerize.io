"""
    This module is to work with the API keys to make the API secured.
"""
...
# Code indicates that the server understands the request but refuses to authorize it.
from starlette.status import HTTP_403_FORBIDDEN

if "API_KEY" in os.environ:
    API_KEY = os.environ["API_KEY"]

# we should comment it out when reading from the env variable
API_KEY = '1234'
...
async def get_api_key(api_key_query: str = Security(api_key_query), api_key_header: str = Security(api_key_header), api_key_cookie: str = Security (api_key_cookie)):
    ...
    if api_key_query == API_KEY:
        ...
        raise HTTPException(status_code = HTTP_403_FORBIDDEN, detail = "Could not validate the credentials")
